...
def points(team, row, col):
    ...
    if team1_score >= 30 or team2_score >= 30:
        if team1_score > team2_score:
            win(1, team1_score, team2_score)
        else:
            win(2, team1_score, team2_score)
        top.destroy()
...
def win(team, score1, score2):
    ...
    exit_button = Button(winner_screen, text="EXIT GAME", command=on_closing, padx=50, pady=50,  bg='black', fg='light blue').grid(row=4, column=0)
...
start_button = Button(root, text="START", padx=100, pady=50, command=start_game, bg='black', fg='light blue').grid(row=0, column=1)
instructions_button = Button(root, text="INSTRUCTIONS", padx=100, pady=50, command=open_instructions, bg='black', fg='light blue').grid(row=1, column=1)
